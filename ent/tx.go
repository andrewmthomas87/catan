...
func (tx *Tx) init() {
	tx.Harbor = NewHarborClient(tx.config)
	tx.Hex = NewHexClient(tx.config)
	tx.NumberToken = NewNumberTokenClient(tx.config)
	tx.Player = NewPlayerClient(tx.config)
	tx.Road = NewRoadClient(tx.config)
	tx.Robber = NewRobberClient(tx.config)
...
// applies a query, for example: Harbor.QueryXXX(), the query will be executed
