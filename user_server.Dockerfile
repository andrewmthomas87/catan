...
COPY . .
RUN go build -v ./cmd/user_server
...
COPY --from=builder /go/src/github.com/andrewmthomas87/catan/user_server .
COPY --from=builder /go/src/github.com/andrewmthomas87/catan/wait-for-it.sh .

RUN chmod +x wait-for-it.sh

CMD ["/go/src/github.com/andrewmthomas87/catan/user_server"]
